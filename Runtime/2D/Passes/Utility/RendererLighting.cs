                ...
                if (shadowCasterGroups != null && shadowCasterGroups.Count > 0)
                {
                    ...
                    for (int group = 0; group < shadowCasterGroups.Count; group++)
                    {
                        ...
                        if (shadowCasters != null)
                        {
                            ...
                            for (int i = 0; i < shadowCasters.Count; i++)
                            {
                                ...
                                if (shadowCaster != null && shadowMaterial != null && shadowCaster.IsShadowedLayer(layerToRender))
                                {
                                    if (shadowCaster.castsShadows)
                                    {
                                        ...
                                        {
                                            if (Vector3.Distance(shadowCaster.transform.position, light.transform.position) > light.pointLightOuterRadius)
                                            {
                                                inRange = false;
                                            }
                                        }
                                        ...
                                        if (inRange)
                                        {
                                            ...
                                            CombineInstance ci = new CombineInstance() { mesh = shadowCaster.mesh, transform = baseTransform * shadowCaster.transform.localToWorldMatrix };
                                            combine.Add(ci);
                                        }
                                    }                                
                                }
                            ...
                            {
                                Mesh m = new Mesh();
                                m.CombineMeshes(combine.ToArray(), true, true);
                                cmdBuffer.DrawMesh(m, shadowCasters[0].transform.localToWorldMatrix, shadowMaterial);
                            }
                            ...
                            if (matrices.Count > 0)
                            {
                                // couldn't get instanced rendering to work alas :(
                                /*SpriteRenderer renderer = shadowCasters[0].GetComponent<SpriteRenderer>();
                                Mesh mm = new Mesh();
                                Vector3[] verts = new Vector3[renderer.sprite.vertices.Length];
                                for (int ii = 0; ii < renderer.sprite.vertices.Length; ii++) verts[ii] = renderer.sprite.vertices[ii];
                                mm.vertices = verts;
                                int[] tris = new int[renderer.sprite.triangles.Length];
                                for (int ii = 0; ii < renderer.sprite.triangles.Length; ii++) tris[ii] = renderer.sprite.triangles[ii];
                                mm.triangles = tris;*/
                                removeSelfShadowMaterial.enableInstancing = true;
                                cmdBuffer.DrawMeshInstanced(shadowCasters[0].mesh, 0, removeSelfShadowMaterial, -1, matrices.ToArray());
                            }
                        }
                    }
                }
